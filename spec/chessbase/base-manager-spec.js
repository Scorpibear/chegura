...
describe("baseManager", () => {
  ...
  describe("getFen", () => {
    it("calls bestmovedb.getFen", () => {
      ...
      expect(bestmovedb.getFen).toHaveBeenCalledWith({
        fen: "abc",
        depth: 100,
      });
    });
    it("adds cp (centipawns) property if not provided by bestmovedb", () => {
      spyOn(bestmovedb, "getFen").and.returnValue({
        fen: "abc",
        score: 0.42,
        depth: 50,
      });
      const fenData = baseManager.getFen({ fen: "abc" });
      expect(fenData.cp).toBe(42);
    });
  });
  describe("index", () => {
    it("calls converter.json2fenbase", async () => {
      ...
      expect(converter.json2fenbase).toHaveBeenCalledWith(
        baseManager.getBase(),
        bestmovedb
      );
    });
