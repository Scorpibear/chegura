describe("converter", () => {
  ...
  describe("json2fenbase", () => {
    ...
    it("skips position if evaluation is absent", () => {
      const jsonbase = { m: "", s: [{ m: "e4", e: { v: 0.1, d: 20 } }] };
      spyOn(console, "error");
      spyOn(fenbase, "add");
    ...
    it("calls once for one bestmove in base", () => {
      ...
      spyOn(fenbase, "add");
      converter.json2fenbase(jsonbase, fenbase);
      expect(fenbase.add).toHaveBeenCalledWith({
        fen,
        bestMove: "e4",
        score: 0.1,
        depth: 20,
    ...
    it("logs stats after defined amount of positions analysis and in the end", () => {
      converter.logPeriod = 10;
      const samplePosition = { m: "e4", e: { v: 0.1, d: 20 } };
      const jsonbase = { m: "", e: { v: 0.1, d: 20 } };
      for (
        let i = 0, position = jsonbase;
        i < converter.logPeriod;
        i++, position = position.s[0]
      ) {
  ...
  describe("movesToFen", () => {
    it("works for start", () => {
      expect(converter.moves2fen([])).toBe(
        "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"
      );
    });
    it("works for 3 moves", () => {
      expect(converter.moves2fen(["e4", "d5", "Nf3"])).toBe(
