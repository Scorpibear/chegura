...
describe("requestProcessor", () => {
  const stub = () => {
    /* just an empty stub for tests to later spyOn and mock */
  };
  const analyzer = { analyzeLater: () => Promise.resolve() };
  const baseManager = {
    getBaseAsString: stub,
    getBase: stub,
    getFenBase: stub,
    getFen: stub,
  };
  ...
  describe("analyze", () => {
    it("send moves to analyzer", () => {
      const req = {
        method: "POST",
        on: (event, handler) => handler(JSON.stringify({ moves })),
      };
    ...
    it("logs error if moves where not posted", () => {
      const req = {
        method: "POST",
        on: (event, handler) => handler(JSON.stringify({})),
      };
    ...
    it("log error and do not crash when incorrect body received", () => {
      const req = {
        method: "POST",
        on: (event, handler) => handler("random body"),
      };
      spyOn(console, "error").and.stub();
      requestProcessor.analyze(req, res);
  ...
  describe("getFenData", () => {
    it("use info from baseManager", () => {
      ...
      spyOn(res, "end");
      requestProcessor.getFenData(
        { url: "/api/fenData?fen=some valid FEN" },
        res
      );
